function stringStyle(string) {
  const {
    fontFamily, fontSize, fontWeight, fontStyle, textDecoration,
    textAlign, stroke, letterSpacing, lineSpacing
  } = string
  if (process.env.NODE_ENV !== 'production') {
    if (!fontFamily || !fontSize || typeof fontSize !== 'number') {
      throw new Error(`Cloudinary :: text overlay style requires a fontFamily and fontSize, received: ${string} - see http://cloudinary.com/documentation/image_transformations#adding_text_captions`)
    }
    if (
...
export function compileFormat(format) {
  if (process.env.NODE_ENV !== 'production') {
    ...
    if (format && !formatOptions.includes(format)) {
      throw new Error(`Cloudinary :: format received ${format}, should be 'auto' or one of ['${formatOptions.join('\', \'')}']`)
    }
  }
  return format ? `.${format}` : ''
}

export function compileTransforms(options) {
  return Object.keys(options)
    .map(key => compileOption(key, options[key]))
    .join(',')
}

export function compileOption(key, value) {
  switch (key) {
    ...
    case 'overlay':
      return 'l_' + (typeof value === 'string'
          ? value
          : `text:${value.publicId || stringStyle(value)}:${encodeURIComponent(value.text)}`
      )
